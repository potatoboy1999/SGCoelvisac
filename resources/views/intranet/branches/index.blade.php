...
@section('style')
    ...
    <link rel="stylesheet" href="{{asset('css/intranet/branches.css')}}">
    <style>
        @foreach ($branches as $branch)
            .branch{{$branch->id}}{
                background: {{$branch->color}}!important;
            }
        @endforeach
        /* {} */
    </style>
...
<div class="modal fade" id="deleteBranchModal" data-coreui-backdrop="static" data-coreui-keyboard="false">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">Eliminar Sede</div>
            <div class="modal-body">
                ...
                <div class="modal-area modal-form">
                    <form id="form_delete" action="{{route('branches.delete')}}" method="POST">
                        @csrf
                        <p>¿Estás seguro que quieres eliminar esta sede?</p>
                        <input type="hidden" name="reunion_id" value="">
                    </form>
                </div>
                <div class="modal-area modal-success" style="display: none">
                    <p>¡La sede ha sido eliminada correctamente!</p>
                </div>
                <div class="modal-area modal-error" style="display: none">
                    <p class="text-danger" id="error_msg">Ha ocurrido un error al eliminar la sede</p>
                </div>
            </div>
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <div class="mb-3 overflow-auto">
                    <table id="branches_tbl" class="table table-bordered m-0 cell-border">
                        <thead>
                            <tr>
                                <th class="h-name bg-dark text-white">Nombre</th>
                                <th class="h-color bg-dark text-white" width="100">Color</th>
                                <th class="h-actions bg-dark text-white text-center" width="150">Acciones</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($branches as $branch)
                            <tr class="row-reunion" reunionid="{{$branch->id}}">
                                ...
                                <td class="d-actions align-middle text-center">
                                    ...
                                    <a href="#" class="text-white btn btn-danger btn-sm btn-remove" data-id="{{$branch->id}}">
                                        <svg class="icon">
...
<script>
    var new_popup = "{{route('branches.new')}}";
    var edit_popup = "{{route('branches.edit')}}";
    var save_new = "{{route('branches.save.new')}}";
    var save_edit = "{{route('branches.save.update')}}";
</script>
@endsection